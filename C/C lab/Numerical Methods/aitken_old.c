...
int main() {
	...
	do {
		...
		if (iteration == 1) {
	    	printf("%3d\t%9.7f\t%9.7f\t%9.7f\t   ----\n", iteration, prev_x_atk, f(prev_x_atk), error);
		}
		else {
    		printf("%3d\t%9.7f\t%9.7f\t%9.7f\t%9.7f\n", iteration, prev_x_atk, f(prev_x_atk), error, fabs(log(error) / log(prev_error)));
    	}
    	fprintf(file, "%lf %lf\n", prev_x, f(prev_x));
    	++iteration;
    	prev_error = error;
    	error = fabs(x - prev_x);
    	printf("%3d\t%9.7f\t%9.7f\t%9.7f\t%9.7f\n", iteration, x, f(x), error, fabs(log(error) / log(prev_error)));
    	fprintf(file, "%lf %lf\n", x, f(x));
    	++iteration;
    	prev_error = error;
    	error = fabs(next_x - x);
    	printf("%3d\t%9.7f\t%9.7f\t%9.7f\t%9.7f\n", iteration, next_x, f(next_x), error, fabs(log(error) / log(prev_error)));    	
    	fprintf(file, "%lf %lf\n", next_x, f(next_x));
